        ...
        <div class="flex-1 flex flex-col">
            ...
            <header class="bg-white shadow">
                <div class="flex justify-between items-center px-4 py-2">
                    <div>
                        <h1 class="text-lg font-semibold">Positions</h1>
                    </div>
            ...
            <main class="p-6 bg-gray-100 min-h-screen">
                <div class="bg-white p-6 rounded-lg shadow-md">
                    <h2 class="text-2xl font-bold mb-4">Contract Types</h2>
                    <a href="{{ route('contract-types.create') }}" class="btn btn-primary mb-4 bg rounded-md bg-blue-600 px-4 py-2">Add Contract Type</a>
                    @if (session('success'))
                    ...
                    <table class="min-w-full bg-white">
                        ...
                        <tbody>
                            @foreach ($contractTypes as $contractType)
                                <tr>
                                    <td class="border px-4 py-2">{{ $contractType->id }}</td>
                                    <td class="border px-4 py-2">{{ $contractType->name }}</td>
                                    <td class="border px-4 py-2">
                                        <a href="{{ route('contract-types.edit', $contractType->id) }}" class="btn btn-warning">Edit</a>
                                        <form action="{{ route('contract-types.destroy', $contractType->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('DELETE')
