        ...
        <div class="flex-1 flex flex-col">
            ...
            <header class="bg-white shadow">
                <div class="flex justify-between items-center px-4 py-2">
                    <div>
                        <h1 class="text-lg font-semibold">Edit Contract Type</h1>
                    </div>
            ...
            <main class="p-6 bg-gray-100 min-h-screen">
                <div class="bg-white p-6 rounded-lg shadow-md">
                    ...
                    <form action="{{ route('contract-types.update', $contractType->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <!-- Contract Type Name -->
                        <div class="mb-4">
                            <label for="name" class="block text-sm font-medium text-gray-700">Contract Type Name</label>
                            <input type="text" name="name" id="name" value="{{ $contractType->name }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                        </div>

                        <!-- Submit Button -->
                        <div class="flex justify-end">
                            <button type="submit" class="btn btn-primary bg-blue-600 text-white px-4 py-2 rounded-md">Update Contract Type</button>
                        </div>
                    </form>
                </div>
            </main>
